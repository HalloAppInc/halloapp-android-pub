...
public class GroupsListResponseIq extends HalloIq {
    ...
    public static GroupsListResponseIq fromProto(GroupsStanza groupsStanza) {
        ...
        for (GroupStanza groupStanza : groupsStanza.getGroupStanzasList()) {
            ...
            groupInfos.add(new GroupInfo(groupStanza.getGroupType(), new GroupId(groupStanza.getGid()), groupStanza.getName(), groupStanza.getDescription(), groupStanza.getAvatarId(), b, new ArrayList<>(), groupStanza.hasExpiryInfo() ? groupStanza.getExpiryInfo() : null));
        }
