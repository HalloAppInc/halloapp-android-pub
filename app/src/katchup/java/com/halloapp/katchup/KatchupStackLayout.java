...
public class KatchupStackLayout extends ConstraintLayout {
    ...
    private final ArrayList<KatchupPostViewHolder> holders = new ArrayList<>();
    private final List<KatchupPost> moments = new ArrayList<>();
    private boolean cardAnimationInProgress = false;

    public KatchupStackLayout(@NonNull Context context) {
        super(context);
        init();
    }

    public KatchupStackLayout(@NonNull Context context, @Nullable @org.jetbrains.annotations.Nullable AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public KatchupStackLayout(@NonNull Context context, @Nullable @org.jetbrains.annotations.Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onDetachedFromWindow() {
        ...
        for (KatchupPostViewHolder holder : holders) {
            holder.markDetach();
        }
    }
    ...
    public void load(KatchupPostViewHolder.KatchupViewHolderParent parent) {
        holders.add(new KatchupPostViewHolder(findViewById(R.id.top), parent));
        holders.add(new KatchupPostViewHolder(findViewById(R.id.middle), parent));
        holders.add(new KatchupPostViewHolder(findViewById(R.id.bottom), parent));

        holders.get(1).itemView.setRotation(CARD_TILT_ANGLE_DEG);
    }

    public void bindTo(@NonNull List<KatchupPost> moments) {
        if (moments.size() == 0) {
            this.moments.clear();
            return;
        }
